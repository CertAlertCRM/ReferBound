...
// Partner gaps — the mirror, not a megaphone. Every number here is the
...
export function PartnerGaps({ onConvert }: { onConvert: (p: Prospect) => void }) {
  const { toast } = useUI();
  const [d, setD] = useState<Gaps | null>(null);
  const [open, setOpen] = useState(false);
  const [adding, setAdding] = useState(false);
  const [form, setForm] = useState({ name: "", company: "", email: "", phone: "", partner_type: "lender" });

  async function load() {
    const r = await fetch("/api/partner-gaps");
  ...
  async function dismiss(id: string) {
    await fetch(`/api/partner-gaps?id=${encodeURIComponent(id)}`, { method: "DELETE" });
    load();
  }
  ...
  async function addAsContact(p: Prospect) {
    ...
    if (res.ok) {
      await fetch(`/api/partner-gaps?id=${encodeURIComponent(p.id)}`, { method: "DELETE" });
      load();
    } else {
    ...
    <section className="card p-5 space-y-4">
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span className="font-semibold">Partner gaps</span>
