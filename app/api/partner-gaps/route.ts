...
import { concentration, PROSPECT_STATUSES } from "@/lib/partner-gaps";
import { PARTNER_TYPES } from "@/lib/config";

export const dynamic = "force-dynamic";

// Partner gaps: the agent's own numbers (where their referrals actually come
// from), plus the two lists worth acting on — loan officers a realtor named on
// their deal, and people missing from a partner's team.
...
export async function GET() {
  ...
  const names = new Map<string, string>(
    ((partners ?? []) as any[]).map((p) => [String(p.id), String(p.name ?? "")])
  );
